class AES(object):
    ...
    rsbox = [0x52, 0x09, 0x6a, 0xd5, 0x30, 0x36, 0xa5, 0x38, 0xbf, 0x40, 0xa3, 0x9e, 0x81, 0xf3, 0xd7, 0xfb,  # 00
    ...
    def expandKey(self, key, size, expandedKeySize):
        ...
        while currentSize < expandedKeySize:
            ...
            if currentSize % size == 0:
                t = self.KeySchedule(t, rconIteration)
                rconIteration += 1
    ...
    def galois_multiplication(self, num1, num2):
       
        p = 0
        for counter in range(8):
            if num2 & 1: p ^= num1
            hi_bit_set = num1 & 0x80
            num1 <<= 1
            num1 &= 0xFF
            if hi_bit_set:
                num1 ^= 0x1b
...
def encryptData(key, data, mode=AESOperation.modeOfOperation["CBC"]):
    key = key

    if mode == AESOperation.modeOfOperation["CBC"]:
        data = append_PKCS7_padding(data)
    keysize = len(key)
    assert keysize in AES.keySize.values(), 'invalid key size: %s' % keysize
    iv = [ord(i) for i in os.urandom(16)]
    obj = AESOperation()
    (mode, length, ciph) = obj.encrypt(data, mode, key, keysize, iv)
    return ''.join(map(chr, iv)) + ''.join(map(chr, ciph))


def decryptData(key, data, mode=AESOperation.modeOfOperation["CBC"]):
    key = key
    keysize = len(key)
    assert keysize in AES.keySize.values(), 'invalid key size: %s' % keysize
    iv = map(ord, data[:16])
    ...
    obj = AESOperation()
    decr = obj.decrypt(data, None, mode, key, keysize, iv)
    if mode == AESOperation.modeOfOperation["CBC"]:
...
def generateRandomKey(keysize):
    if keysize not in (16, 24, 32):
        msg = 'Invalid keysize, %s. Should be one of (16, 24, 32).'
        raise ValueError(msg , keysize)
    return os.urandom(keysize)


def runAES(cleartext, keyin, keysize=16, modeName="CBC"):
    '''Test with random key, choice of mode.'''
    print '\nMode:', modeName
    print '\nCleartext:', cleartext
    #key = generateRandomKey(keysize)
    key = keyin
    keychr = [chr(x) for x in key]
    print '\nKey(in string): ',''.join(keychr)
    
    print '\nKey: ',[x for x in key]
    print '\nKey Size: %d bytes'%len(key)    

    mode = AESOperation.modeOfOperation[modeName]
    ...
    str5 = ""
    for x in cipher:
        str1 = str1 + str(x.strip().split(','))
    for y in str1:
        str2 = str2 + str(y.strip('['))
    for z in str2:
        str3 = str3 + str(z.strip(']'))
    for p in str3:
        str4 = str4 + str(p.strip('\\x'))
    for q in str4:
        str5 = str5 + str(q.strip('\''))
...
if __name__ == "__main__":
    ...
    cleartext = "This is a test string to check the execution of AES Algorithm in CBC Mode. The CBC mode divides the data blocks and then encrypts each block using the previously encrypted block. The first block is encrypted using IV which get appended at the start of the entire ciphertext generated using the algorithm."
    ...
    runAES(cleartext, cypherkey, 16, "CBC")
